...
const IfxTabs = class {
    ...
    async componentDidLoad() {
        if (!domUtils.isNestedInIfxComponent(this.el)) {
            ...
            tracking.trackComponent("ifx-tabs", await framework);
        }
        this.updateBorderAndFocus();
        this.updateScrollButtons();
    ...
    render() {
        return (index.h("div", { key: 'c83e230d8b775b619cc3bb5ff5c64946d023ede4', "aria-label": "navigation tabs", class: `tabs ${this.internalOrientation} ${this.fullWidth ? "full-width-enabled" : ""}` }, this.internalOrientation === "horizontal" ? (index.h("div", { class: "tabs-container" }, index.h("ifx-icon-button", { shape: "round", variant: "tertiary", icon: "chevronLeft16", size: "s", disabled: false, "aria-label": "Scroll tabs left", onClick: () => this.scrollLeft(), class: `scroll-button scroll-left ${!this.canScrollLeft ? "hidden" : ""}` }), index.h("ul", { role: "tablist", class: "tabs-list scrollable", ref: (el) => (this.tabsListElement = el), onScroll: () => this.onTabsListScroll() }, this.tabObjects?.map((tab, index$1) => (index.h("li", { class: this.getTabItemClass(index$1), ref: (el) => (this.tabHeaderRefs[index$1] = el), onMouseDown: (event) => event.preventDefault(), onClick: () => this.handleClick(tab, index$1), "aria-selected": index$1 === this.internalActiveTabIndex ? "true" : "false", "aria-disabled": tab.disabled ? "true" : "false", role: "tab" }, tab?.icon ? index.h("ifx-icon", { icon: tab.icon }) : "", tab?.header))), index.h("div", { class: "active-border" })), index.h("ifx-icon-button", { shape: "round", variant: "tertiary", icon: "chevronRight16", size: "s", disabled: false, "aria-label": "Scroll tabs right", onClick: () => this.scrollRight(), class: `scroll-button scroll-right ${!this.canScrollRight ? "hidden" : ""}` }))) : (index.h("ul", { role: "tablist", class: "tabs-list" }, this.tabObjects?.map((tab, index$1) => (index.h("li", { class: this.getTabItemClass(index$1), ref: (el) => (this.tabHeaderRefs[index$1] = el), onMouseDown: (event) => event.preventDefault(), onClick: () => this.handleClick(tab, index$1), "aria-selected": index$1 === this.internalActiveTabIndex ? "true" : "false", "aria-disabled": tab.disabled ? "true" : "false", role: "tab" }, tab?.icon ? index.h("ifx-icon", { icon: tab.icon }) : "", tab?.header))), index.h("div", { class: "active-border" }))), index.h("div", { key: 'fee72b044a7dcb5e7a3bfb1b7fdbfb1c0500b780', class: "tab-content" }, Array.from(this.tabObjects).map((_, index$1) => (index.h("div", { style: {
                display: index$1 === this.internalActiveTabIndex ? "block" : "none",
            } }, index.h("slot", { name: `tab-${index$1}` })))))));
    }
...
//# sourceMappingURL=ifx-tabs.entry.cjs.js.map
